...
logger = logging.getLogger("Day10App")

# ---------------------------------------------------------
# Database Constants & Setup (for Startup Lifespan Seeding)
# ---------------------------------------------------------
...
async def lifespan(app: FastAPI):
    """
    Lifespan context manager that handles database initialization on startup.
    Reads db-setup.sql and seeds the SQLite database if it is not initialized.
    """
    conn = None
    try:
    ...
    yield  # Control handed over to FastAPI server
...
# ---------------------------------------------------------
app = FastAPI(
    title="Day 10: Input Validation & Error Handling",
    description="FastAPI exercise focusing on raw SQL, robust validation, and custom error parsing.",
    version="1.0.0",
    lifespan=lifespan
)
...
def serialize_validation_errors(errors: list) -> list:
    ...
    Exception objects (like ValueError in Pydantic v2's ctx['error']) to strings.
    """
    cleaned = []
    for err in errors:
        err_copy = dict(err)
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    """
    # Safeguard body extraction: make sure body is JSON serializable
    body_payload = None
    try:
    ...
    # Sanitize and serialize validation errors to strip raw Python Exceptions
    raw_errors = exc.errors()
    error_details = serialize_validation_errors(raw_errors)
    
    # Log validation failure details for server-side troubleshooting
    logger.error(
...
@app.get("/", tags=["Root"])
def read_root():
    return {"message": "Welcome to the Day 10 Tasks API. Go to /docs to view interactive documentation."}
